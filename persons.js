...
class Persons {
  ...
  #meAndMySiblings(id){
    ...
    let parent1id=person.parent1id
    let parent2id=person.parent2id
    ...
    for(let pid of keys){
      ...
      if(person.parent1id||person.parent2id){
        if(person.parent1==parent1id || person.parent2id==parent2id){
          siblings.push(this.persons[pid])
        }
      }
    }
    return siblings
  ...
  getNextSibling(personId){
    let siblings=this.#meAndMySiblings(personId)
  }
  #drawBackGround(){
